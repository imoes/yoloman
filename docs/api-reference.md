...
2. **A host is normally addressed by its agent id**, not by its name — names are reused across the
   lifetime of a fleet, and an id keeps pointing at the same host after a rebuild. `GET
   /api/v1/agents` maps names to ids. There are deliberate exceptions, and they are marked where they
   occur: the **plan-run** surface takes a host *name*, because that is what a human or a model
   actually knows, and the **provisioning** routes take a hostname for a machine that has no agent
   yet. Where a name is accepted, the reuse caveat applies to the caller instead of to the API.
...
Generated content, kept separate from the hand-arranged dashboards (`/api/v1/dashboards`): a model-authored layout must not be mistaken for one a person built, and neither should overwrite the other.
...
As with Codex, the credentials land in the user's server-side chat home and no token is returned to the client.
...
The counterpart is `.../claude/complete`, not a poll: this flow hands the user a code rather than waiting on the provider.

#### `POST /api/v1/chat/oauth/codex/poll/{session_id}`

Ask whether that authorisation has completed yet.

On success the credentials are written into **this user's** chat home on the server and the response is `{"status": "authorized"}` and nothing else — **the tokens are never returned to the client.** A browser that never receives a token cannot leak one.

Any other status is returned as the provider gave it, so a caller can distinguish "still waiting" from "denied" instead of retrying a decision that has already been made. 400 when the flow itself is invalid or expired.
...
Poll `.../codex/poll/{session_id}` afterwards. 502 when the provider itself cannot be reached — a distinct case from a rejected authorisation, which the poll reports.
...
Every field answers as "what would be used right now": an unset `default_backend` reads as the server's configured one rather than as `null`, so a caller never has to know the fallback rules to display the effective value. `models` maps a backend to the model chosen for it.

#### `PATCH /api/v1/chat/prefs`

Change chat preferences; omitted fields stay as they are.

`models` is **merged**, not replaced — setting the model for one backend does not forget the others. 422 when `default_backend` is not a known backend.
...
Sessions are per-user and not shared; another user's session is invisible rather than forbidden. Each row carries its message count, so a client need not fetch a transcript to show how long a conversation is.

#### `POST /api/v1/chat/sessions`

Start a chat session, pinned to one backend for its lifetime.

The backend comes from the body, else this user's preference, else the server default; it must be one of the known backends (422). **It cannot be changed afterwards** — see `send_message` for why a session is pinned rather than re-selectable per message.
...
Only the caller's own sessions (404 otherwise — not 403, because whether someone else's session id exists is not this caller's business). The system prompt is **not** part of the transcript: it is injected per request and never stored, so it can be improved without rewriting history.
...
**The session's backend is authoritative.** A `backend` in the body is accepted only if it matches the session's; otherwise **409**, naming the pin. Letting a per-request field redirect a pinned session is how a conversation ends up half-answered by two different models, each unaware of the other's turns.

What happens in order: the user's turn is persisted first (so a crash mid-answer cannot lose the question), the whole transcript is loaded, and a **non-persisted** system prompt is prepended. That prompt is not stored on purpose — it can be improved without rewriting what people actually said.

With an agentic backend the model may **call fleet tools**, executed in-process against a session held open for the whole loop. Those calls do what they say: they are the same actions the UI performs, subject to the same authorisation as the caller. This endpoint is therefore a write surface, whatever it looks like.

422 for empty content, for an unknown backend, or when the backend cannot be built (missing credentials, unreachable endpoint) — the message says which.
...
Every runbook plan in the catalogue — and this is the third thing called a plan.

Three exist in this API and they are not variants of one idea:

- **A runbook plan (this one)** is a step list you *execute* against a host. It has a `/run`. - **An orchestration plan** (`/api/v1/orchestration/plans`) is *desired state*: it applies by being linked to a scope, and nothing runs when you create it. - **A stored plan** (`/api/v1/plans/stored`) is the canonical database-backed store for the same documents as this catalogue, keyed by tool prefix (ansible, salt, puppet, chef). Both exist during the move to one store, which is a transition rather than a design.

This listing comes from the in-memory catalogue; `POST /api/v1/plans/reload` refreshes it.
...
The canonical plan store: documents in the database, keyed by tool prefix.

`prefix` filters to one of ansible, salt, puppet or chef (400 for anything else). Each entry carries a `version` and a `content_hash`, so a caller can tell an edit from a re-import without reading the body.

This is the store the file catalogue above is moving into. While both exist, a plan can appear in each — check the version and hash rather than assuming they agree.
...
Store a plan document, translating it if it comes from another tool.

`source_format` is either native (`yaml`, `json`) or a foreign format (`salt`, `puppet`, `chef`) that is converted on the way in. A new version is created rather than the old one overwritten, so an import can be compared with what it replaced.

422 with the engine's own message when the source cannot be parsed or translated — named now, at import, rather than when someone tries to run it.
...
One runbook plan with its steps — what would run, in order, before you run it.
...
A written briefing for this plan, generated by the language model.

What the plan does, in prose, for the run dialog — so an operator is not deciding from a step list alone. Two deliberate properties:

- **It never blocks the dialog.** A tighter timeout than the chat's (90s), and on any failure the response carries `error` with `markdown: null` rather than a 5xx. A run dialog that hangs on an overloaded model endpoint is worse than one without a briefing. - **Thinking is disabled** for this call. The output is prose for a human; reasoning tokens would consume the budget without reaching the reader. Note what is *not* done: no token cap. A cap is what produces a truncated answer — the model bounds itself.

Advisory only. The briefing is generated text and does not gate, alter or approve the run.
...
Run a plan against a host. This is the "take plan X, run it against host Y" surface.

**The host is named, not id-addressed** — `agent` is the host's name — because that is what a human or a model actually knows. It is the deliberate exception to this API's general rule; the caveat that motivates the rule still applies, so if a host has been rebuilt under the same name, this reaches the new one.

**`dry_run` defaults to `false` here** — the opposite of every resource `apply`, where it defaults to `true`. Same word, opposite default, in one API: a plan run is an explicit act someone asked for, while a resource apply is often a client filling a form. Send it explicitly and do not carry an assumption over from the other surface.

Four refusals, each naming its reason: 404 for an unknown plan or host, **403** when the caller may not manage this host, 422 when the host has no reachable address, and 422 with the plan engine's own message when a step is invalid or a required parameter is missing.

Variables come from the **database**, scope-merged (global < group < OU < site < host) — there is no filesystem `host_vars` layer any more, so one fact has one source. Explicit `params` override them.

Returns as soon as the run is recorded: `plan_run_id` and a status. The run continues server-side; follow it through the run endpoints rather than expecting a result here.

...
const SIDECAR = '/etc/agentic-mcp/websites/caddy/caddy.json';

/**
 * Caddy config snapin — single-file Caddyfile editor (one-object, like HAProxy).
 * The whole Caddyfile is rendered from values via the caddy Class-B template;
 * a `sites` table drives per-site blocks (domain, reverse-proxy upstream OR file
 * root, tls). Caddy does automatic HTTPS by default, so the "SSL" surface is
 * mostly just the ACME email + an optional per-site tls override. Values persist
 * as a JSON sidecar; a pre-existing hand-written Caddyfile shows read-only until
 * the operator opts into form management. Apply = template_render + sidecar via
 * state/apply, then `caddy validate` and reload.
 */
@Component({
  selector: 'app-caddy-config',
  standalone: true,
  imports: [MatIconModule, MatButtonModule, ParamFormComponent],
  template: `
    <div class="bm-hap">
      @if (loading()) { <p class="bm-dim">Loading Caddy config…</p> }
      @else {
        <div class="bm-hap-head">
          <strong>Caddy</strong> <span class="bm-dim">· {{ cfgPath }}</span>
        ...
        @if (!managed()) {
          <p class="bm-dim">This Caddyfile was not created here. Editing it as values would replace the whole file with a template render — review the current file first.</p>
          ...
          <p class="bm-dim">Edit the values — the whole Caddyfile is rendered from them. Caddy handles HTTPS automatically per site.</p>
          <app-param-form [params]="schema()!" [initial]="values()" [agentId]="agentId()" (valuesChange)="onValues($event)" />
          @if (rendered()) {
            <p class="bm-dim">Rendered Caddyfile (would be written):</p>
...
export class CaddyConfigComponent {
  private agentService = inject(AgentService);
  agentId = input.required<string>();

  readonly cfgPath = CADDYFILE;
  ...
  private loadRaw(): void {
    this.agentService.callTool(this.agentId(), 'command', { argv: ['cat', CADDYFILE] }).subscribe({
      next: (resp) => { this.raw.set((resp.result as { data?: { stdout?: string } })?.data?.stdout || '(empty)'); this.managed.set(false); this.loading.set(false); this.loaded.set(true); },
  ...
  preview(): void {
    ...
    this.agentService.renderTemplate(this.agentId(), this.tplBody, this.values(), CADDYFILE).subscribe({
  ...
  save(): void {
    ...
    const resources: ConfigResource[] = [
      { type: 'template_render', path: CADDYFILE, template: this.tplBody, values: this.values() },
    ...
    this.agentService.stateApply(this.agentId(), resources, this.dryRun()).subscribe({
      next: (resp) => {
        ...
        this.validateAndReload((ok, detail) => {
          this.busy.set(false);
          if (this.dryRun()) { this.msg.set(`Preview: ${n} change(s), caddy validate ${ok ? 'OK' : 'FAILED'} — nothing written.`); if (!ok) this.err.set(detail); return; }
          if (!ok) { this.err.set(`Written but caddy validate FAILED: ${detail}`); return; }
          this.msg.set(`Saved ${n} change(s), Caddy reloaded.`);
        });
      },
  ...
  private validateAndReload(done: (ok: boolean, detail: string) => void): void {
    this.agentService.callTool(this.agentId(), 'command', { argv: ['caddy', 'validate', '--adapter', 'caddyfile', '--config', CADDYFILE] }).subscribe({
      next: (resp) => {
        const r = resp.result as { data?: { rc?: number; stderr?: string } };
        const ok = (r?.data?.rc ?? 1) === 0;
        if (this.dryRun() || !ok) { done(ok, r?.data?.stderr || ''); return; }
        this.agentService.callTool(this.agentId(), 'systemd', { name: 'caddy', state: 'reloaded' }).subscribe({
          next: () => done(true, ''), error: () => done(true, ''),
        });
      },
      error: () => done(false, 'caddy validate could not run'),
    });
  }
}

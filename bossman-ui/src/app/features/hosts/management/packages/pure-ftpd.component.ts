...
const PASSWD = '/etc/pure-ftpd/pureftpd.passwd';

/**
 * Pure-FTPd snapin — manage virtual FTP users (pure-pw / PureDB). Each user's
 * home directory is its share, chosen via the directory browser. Adding the
 * first user bootstraps PureDB auth (writes conf/PureDB + the auth/ symlink)
 * so pure-ftpd actually consults the virtual-user database, then restarts the
 * service. Virtual users are backed by the system `ftp` account.
 */
@Component({
  selector: 'app-pure-ftpd',
  standalone: true,
  ...
  template: `
    <div class="bm-ftp">
      ...
      @else {
        <div class="bm-head">
          <span class="bm-dim">Pure-FTPd virtual users (PureDB)</span>
...
export class PureFtpdComponent {
  ...
  reload(): void {
    ...
    this.agentService.callTool(this.agentId(), 'command', { argv: ['sh', '-c', `pure-pw list -f ${PASSWD} 2>/dev/null || true`] }).subscribe({
      next: (resp) => {
        ...
        for (const line of out.split('\n')) {
          ...
          us.push({ name: parts[0], home: (parts[1] || '').replace(/\/\.\/?$/, '') });
        }
        this.users.set(us);
      },
  ...
  add(): void {
    ...
    // shell-escape single quotes in the password for the printf.
    const q = (s: string) => `'${s.replace(/'/g, `'\\''`)}'`;
    // Bootstrap PureDB auth (idempotent), then create the user + rebuild the db.
    const script =
      `set -e; ` +
      `mkdir -p ${q(home)}; ` +
      `[ -f /etc/pure-ftpd/conf/PureDB ] || echo ${PASSWD.replace('.passwd', '.pdb')} > /etc/pure-ftpd/conf/PureDB; ` +
      `ln -sf ../conf/PureDB /etc/pure-ftpd/auth/50puredb; ` +
      `printf '%s\\n%s\\n' ${q(pass)} ${q(pass)} | pure-pw useradd ${q(name)} -f ${PASSWD} -u ftp -g nogroup -d ${q(home)} -m; ` +
      `chown -R ftp:nogroup ${q(home)} 2>/dev/null || true; ` +
      `(systemctl restart pure-ftpd 2>/dev/null || systemctl restart pure-ftpd-mysql 2>/dev/null || true)`;
    ...
    this.agentService.callTool(this.agentId(), 'command', { argv: ['sh', '-c', script] }).subscribe({
      next: (resp) => {
        ...
        if ((d?.rc ?? 0) !== 0) { this.err.set(d?.stderr || 'useradd failed'); return; }
  ...
  del(u: VUser): void {
    this.busy.set(true); this.msg.set(''); this.err.set('');
    const q = (s: string) => `'${s.replace(/'/g, `'\\''`)}'`;
    this.agentService.callTool(this.agentId(), 'command', { argv: ['sh', '-c', `pure-pw userdel ${q(u.name)} -f ${PASSWD} -m`] }).subscribe({

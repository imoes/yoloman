...
export interface CheckRule {
  ...
  /** Set when this rule was generated by a Check template linked to a host group.
   * Then the rule is read-only here (the API answers 409 on write) and the template
   * is the place to change it — the UI greys the row out and names the origin. */
  template_id?: string | null;
  source_template_rule_id?: string | null;
}

/** What a client may send: neither the identity/created_at, nor `is_default`, nor the
 * server-assigned provenance — a caller cannot claim a rule came from a template. */
export type CheckRuleInput = Omit<
  CheckRule,
  'id' | 'created_at' | 'is_default' | 'template_id' | 'source_template_rule_id'
>;

/** ------------------------------------------------------------------------
 * Check templates (API: /api/v1/templates) — a named, reusable bundle of check
 * rules, linked to host groups. Linking MATERIALIZES real CheckRule rows scoped
 * to that group; editing the template re-materializes every link.
 *
 * Called "Check template" throughout the UI, never bare "Template": the UI already
 * has Config templates and Disk images, so the unqualified word would name three
 * different things. The API's entity is `Template`; this is the one place the two
 * vocabularies meet. */
export interface CheckTemplateRule {
  id?: string;
  service_name: string;
  metric: string;
  comparison: CheckRuleComparison;
  ...
  extra_conditions?: Record<string, unknown>[] | null;
...
export interface CheckTemplate {
  ...
  created_at: string;
  rules: CheckTemplateRule[];
  /** Templates whose rules this one also contributes — resolved transitively and
   * cycle-safe on the server (services/templates.py). */
  nested_template_ids: string[];
}
...
/** A template↔host-group link. `host_group` is the group's NAME, not its id. */
export interface CheckTemplateLink {
  id: string;
  template_id: string;
  host_group: string;
}

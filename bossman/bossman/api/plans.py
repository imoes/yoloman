...
async def list_plans(
    ...
    - **A runbook plan (this one)** is a step list you *execute* against a host. It has a `/run`.
    - **An orchestration plan** (`/api/v1/orchestration/plans`) is *desired state*: it applies by
      being linked to a scope, and nothing runs when you create it.
    - **A stored plan** (`/api/v1/plans/stored`) is the canonical database-backed store for the
      same documents as this catalogue, keyed by tool prefix (ansible, salt, puppet, chef). Both
      exist during the move to one store, which is a transition rather than a design.
...
async def plan_briefing(
    ...
    What the plan does, in prose, for the run dialog — so an operator is not deciding from a step
    list alone. Two deliberate properties:

    - **It never blocks the dialog.** A tighter timeout than the chat's (90s), and on any failure
      the response carries `error` with `markdown: null` rather than a 5xx. A run dialog that
      hangs on an overloaded model endpoint is worse than one without a briefing.
    - **Thinking is disabled** for this call. The output is prose for a human; reasoning tokens
      would consume the budget without reaching the reader. Note what is *not* done: no token
      cap. A cap is what produces a truncated answer — the model bounds itself.
...
async def run_plan_route(
    ...
    **The host is named, not id-addressed** — `agent` is the host's name — because that is what a
    human or a model actually knows. It is the deliberate exception to this API's general rule;
    the caveat that motivates the rule still applies, so if a host has been rebuilt under the same
    name, this reaches the new one.

    **`dry_run` defaults to `false` here** — the opposite of every resource `apply`, where it
    defaults to `true`. Same word, opposite default, in one API: a plan run is an explicit act
    someone asked for, while a resource apply is often a client filling a form. Send it
    explicitly and do not carry an assumption over from the other surface.

    Four refusals, each naming its reason: 404 for an unknown plan or host, **403** when the
    caller may not manage this host, 422 when the host has no reachable address, and 422 with the
    plan engine's own message when a step is invalid or a required parameter is missing.

    Variables come from the **database**, scope-merged (global < group < OU < site < host) — there
    is no filesystem `host_vars` layer any more, so one fact has one source. Explicit `params`
    override them.

    Returns as soon as the run is recorded: `plan_run_id` and a status. The run continues
    server-side; follow it through the run endpoints rather than expecting a result here.
    """
    plan = _find_plan_or_404(cache, name)
...
async def list_stored_plans(
    ...
    `prefix` filters to one of ansible, salt, puppet or chef (400 for anything else). Each entry
    carries a `version` and a `content_hash`, so a caller can tell an edit from a re-import
    without reading the body.

    This is the store the file catalogue above is moving into. While both exist, a plan can appear
    in each — check the version and hash rather than assuming they agree.
    """
...
async def create_stored_plan(
    ...
    `source_format` is either native (`yaml`, `json`) or a foreign format (`salt`, `puppet`,
    `chef`) that is converted on the way in. A new version is created rather than the old one
    overwritten, so an import can be compared with what it replaced.

    422 with the engine's own message when the source cannot be parsed or translated — named now,
    at import, rather than when someone tries to run it.
    """
    try:

...
async def config_rollback(
    ...
    Two limits worth knowing. **Only the newest 30 generations are kept** per resource, the same cap
    the docker desired-state model uses, pruned in the same transaction as the insert. A rollback to
    a generation that has been dropped says so — "pruned, the oldest still held is N" rather than
    "no such generation", because a trimmed history and a typo are different problems. And this
    endpoint always merges; the `exact`
